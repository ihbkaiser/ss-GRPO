...
try:
    from distributed_utils import setup_distributed, cleanup_distributed, reduce_sum, reduce_max, rank0_print
except ImportError:
    from distributed_utils_mg import setup_distributed, cleanup_distributed, reduce_sum, reduce_max, rank0_print
...
def evaluate_mask(model, tokenizer, prompts, mask, candidate_layers, args):
    ...
    global_seconds = reduce_max(total_seconds, device)
    global_tokens = reduce_sum(total_tokens, device)
    global_accept = reduce_sum(total_accept, device)
    global_steps = reduce_sum(total_steps, device)
    tokens_per_second = global_tokens / global_seconds if global_seconds else 0.0
    return {
        "skip_layers": skip_layers,
        "seconds": global_seconds,
        "generated_tokens": int(global_tokens),
        "tokens_per_second": tokens_per_second,
        "average_accept_length": global_accept / global_steps if global_steps else 0.0,
...
def main():
    ...
    model = AutoModelForCausalLM.from_pretrained(args.model_dir, torch_dtype="auto", config=config).to(ctx.device).eval()
