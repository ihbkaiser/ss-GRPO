...
def clone_cache(cache, causal_lm=None, *, clone_tensors: bool = False):
    if cache is None:
        return None
    ...
    if hasattr(cache, "key_cache"):
        if clone_tensors:
            new_cache.key_cache = [key.clone() for key in cache.key_cache]
            new_cache.value_cache = [value.clone() for value in cache.value_cache]
        else:
            # Tree verification only needs an independent cache container. Avoid
            # copying the full prefix KV every decode step; DynamicCache appends
            # by assigning new tensors to this container.
    ...
    for layer_idx in range(_cache_layer_count(cache)):
        ...
        _set_cache_layer(
            new_cache,
            layer_idx,
            key.clone() if clone_tensors else key,
            value.clone() if clone_tensors else value,
        )
    return new_cache
...
def cache_seq_length(cache) -> int:
    ...
    key, _ = _get_cache_layer(cache, 0)
...
def forward_tree(
    ...
    tree_attention_mask_4d: torch.Tensor,
    past_key_values,
    position_ids: torch.Tensor,
    *,
